<?php use_helper('Object', 'Javascript') ?>
  ...
  <td class="leftCol"><label><?php echo ($index == 0) ? __('Contact person') : __('Aditional contact') ?></label></td>
  <td class="rightCol">
        <?php echo javascript_tag("function getContact".$index."(text, li){ $('contact_".$index."_id').value = li.id;}") ?>
    <?php echo input_auto_complete_tag('contact['.$index.'][name]', $contact,
        'contact/autocomplete',
        array('autocomplete' => 'off', 'class' => 'form-medium-text'),
        array('use_style'    => true, 'after_update_element' => 'getContact' . $index)
        ) ?>
