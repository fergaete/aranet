...
<?php $i = 0; foreach ($pager->getResults() as $client) : $i++ ?>
        ...
        "actions":"<div class='objectActions'><ul><li><?php echo str_replace('"', "'", link_to(image_tag(sfConfig::get('yui_icons_web_dir') . '/application_form.png', 'alt='.__("View")), '@client_show_by_id?id='.$client->getId())) ?></li><li><?php echo str_replace('"', "'", link_to(image_tag(sfConfig::get('yui_icons_web_dir') . '/application_form_edit.png', 'alt='.__('Edit')), '@client_edit_by_id?id='.$client->getId())) ?></li><li><?php echo str_replace('"', "'", link_to(image_tag(sfConfig::get('yui_icons_web_dir') . '/application_form_delete.png', 'alt='.__('Delete')), '@client_delete_by_id?id='.$client->getId())) ?></li></ul></div>",
