...
class dashboardActions extends sfActions
{
  ...
  public function executeShowGraphic() {
    if ($request->getParameter('graphic_id') != -1) {
      $graphic = GraphicPeer::retrieveByPk($request->getParameter('graphic_id'));
    } else {
      $c = new Criteria();
      $c->add(GraphicPeer::IS_DEFAULT, true);
      $graphic = GraphicPeer::doSelectOne($c);
    ...
    $c->add(GraphicPeer::ID, $request->getParameter('graphic_id', 1));
    $plots = GPlotPeer::doSelect($c);
    for($i = 0; $i < $n; $i++ ) {
      $datax[0][$i] = $ts+$i*$mult;
