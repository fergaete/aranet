 ...
 */
class cashActions extends anActions
{
  ...
  protected function getCashItem()
  {
    if ($request->getParameter('id')) {
      $cash_item = CashItemPeer::retrieveByPk($request->getParameter('id'));
  ...
  public function executeUpdate()
  {
    ...
    $cash_item->setId($request->getParameter('id'));
    $cash_item->setCashItemName($request->getParameter('cash_item_name'));
    $cash_item->setCashItemComments($request->getParameter('cash_item_comments'));
    if ($request->getParameter('cash_item_date'))
    {
      list($d, $m, $y) = sfI18N::getDateForCulture($request->getParameter('cash_item_date'), $this->getUser()->getCulture());
      $cash_item->setCashItemDate("$y-$m-$d");
    }
    $cash_item->setCashItemAmount($request->getParameter('cash_item_amount'));
  ...
  protected function addFiltersCriteria ($c)
  {
    ...
    if (isset($this->filters['cash_name']) && $this->filters['cash_name'] && $this->filters['cash_name'] != sfContext::getInstance()->getI18N()->__('Name') . '...')
    {
