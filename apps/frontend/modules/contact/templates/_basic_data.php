...
<?php use_helper('NumberExtended') ?>
            ...
            <?php echo ($contact->getClient()) ? link_to($contact->getClient()->getFullName(), '@show_client_by_id?id=' . $contact->getClient()->getId()) . "<br/>" : '' ?>
            <?php echo ($contact->getVendor()) ? link_to($contact->getVendor()->getFullName(), '@show_vendor_by_id?id=' . $contact->getVendor()->getId()) . "<br/>" : '' ?>
            <?php else : ?>
            <strong><?php echo __('Main contact') ?></strong>:<br/>
            <?php echo link_to($contact, '@show_contact_by_id?id=' . $contact->getId()) ?><br/>
            <?php endif ?>
